...
run: $(BRA) ## Build and run web server on filesystem changes.
	@APP_NAME="grafo" APP_MODE="development" APP_PROTOCOL="http" APP_PORT="3000" ROOT_URL="http://localhost:3000/" POSTGRES_HOST="" \
	POSTGRES_PORT=5432 POSTGRES_DBNAME="" POSTGRES_USER="" POSTGRES_PASSWORD="" SMTP_HOST="" \
	SMTP_USER="" SMTP_PWD="123456" SMTP_FROM="" SMTP_FROMNAME="Dashboard" ADMIN_USERNAME="admin" ADMIN_SECRET="admin" \
	DATASERVICE_URL="" DATASERVICE_TOKEN="test" INFLUXDB_URL="" INFLUXDB_PWD="" INFLUXDB_USER="" \
	RESOURCE_URL= RESOURCE_LABEL=Aset RESOURCE_TOKEN= \
	NATS_URL= REDIS_HOST=localhost REDIS_PORT=6379 REDIS_PASSWORD= ENABLE_RESOURCE=true ENABLE_BILLING=true READER_URL="" \
	$(BRA) run
