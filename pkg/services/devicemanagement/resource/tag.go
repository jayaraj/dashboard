...
func (service *Service) GetResourceTags(c *contextmodel.ReqContext) response.Response {
	...
	url := fmt.Sprintf("%sapi/resources/%d/tags/search?query=%s&page=%d&perPage=%d", service.cfg.ResourceHost, id, query, page, perPage)
...
func (service *Service) UpdateResourceTags(c *contextmodel.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":resourceId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
	dto := &resource.UpdateResourceTagsMsg{
		ResourceId: id,
	}
	...
	url := fmt.Sprintf("%sapi/resources/%d/tags", service.cfg.ResourceHost, id)
	req := &devicemanagement.RestRequest{
		Url:        url,
		Request:    body,
