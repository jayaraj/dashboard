...
func (hs *HTTPServer) UpdateInventoryConfiguration(c *models.ReqContext) response.Response {
	...
	dto := dtos.UpdateInventoryConfigurationMsg{
		InventoryId: id,
		Type:        config,
	}
	...
	url := fmt.Sprintf("%sapi/inventories/%d/configurations", hs.ResourceService.GetConfig().ResourceUrl, id)
...
func (hs *HTTPServer) GetInventoryConfiguration(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":inventoryId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
	...
	dto := &dtos.GetInventoryConfigurationMsg{
		InventoryId: id,
		Type:        config,
		User: dtos.User{
			UserId: c.UserID,
	...
	url := fmt.Sprintf("%sapi/inventories/%d/configurations/%s", hs.ResourceService.GetConfig().ResourceUrl, id, config)
	req := &resources.RestRequest{
		Url:        url,
		Request:    body,
