...
func (hs *HTTPServer) CreateFixedCharge(c *models.ReqContext) response.Response {
	dto := dtos.CreateFixedChargeMsg{
		OrgId: c.OrgID,
	}
	...
	url := fmt.Sprintf("%sapi/fixedcharges", hs.ResourceService.GetConfig().BillingUrl)
...
func (hs *HTTPServer) UpdateFixedCharge(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":fixedchargeId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
	dto := &dtos.UpdateFixedChargeMsg{
		Id: id,
	}
	if err := web.Bind(c.Req, &dto); err != nil {
		return response.Error(http.StatusBadRequest, "bad request data", err)
	...
	url := fmt.Sprintf("%sapi/fixedcharges/%d", hs.ResourceService.GetConfig().BillingUrl, id)
	req := &resources.RestRequest{
		Url:        url,
		Request:    body,
...
func (hs *HTTPServer) DeleteFixedCharge(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":fixedchargeId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
...
func (hs *HTTPServer) GetFixedChargeById(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":fixedchargeId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
	...
	cmd := dtos.GetFixedChargeByIdMsg{}
	if req.StatusCode != http.StatusOK {
		var errResponse dtos.ErrorResponse
		if err := json.Unmarshal(req.Response, &errResponse); err != nil {
			response.Error(req.StatusCode, "failed unmarshal error ", err)
...
func (hs *HTTPServer) GetFixedCharges(c *models.ReqContext) response.Response {
	url := fmt.Sprintf("%sapi/orgs/%d/fixedcharges", hs.ResourceService.GetConfig().BillingUrl, c.OrgID)
	req := &resources.RestRequest{
		Url:        url,
		Request:    nil,
		HttpMethod: http.MethodGet,
