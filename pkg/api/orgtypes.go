...
func (hs *HTTPServer) CreateOrgType(c *models.ReqContext) response.Response {
	dto := dtos.CreateOrgTypeMsg{}
	...
	url := fmt.Sprintf("%sapi/orgtypes", hs.ResourceService.GetConfig().ResourceUrl)
...
func (hs *HTTPServer) UpdateOrgType(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":orgtypeId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
	dto := &dtos.UpdateOrgTypeMsg{
		Id: id,
	}
	if err := web.Bind(c.Req, &dto); err != nil {
		return response.Error(http.StatusBadRequest, "bad request data", err)
	...
	url := fmt.Sprintf("%sapi/orgtypes/%d", hs.ResourceService.GetConfig().ResourceUrl, id)
	req := &resources.RestRequest{
		Url:        url,
		Request:    body,
...
func (hs *HTTPServer) DeleteOrgType(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":orgtypeId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
...
func (hs *HTTPServer) GetOrgTypeById(c *models.ReqContext) response.Response {
	id, err := strconv.ParseInt(web.Params(c.Req)[":orgtypeId"], 10, 64)
	if err != nil {
		return response.Error(http.StatusBadRequest, "id is invalid", err)
	}
...
func (hs *HTTPServer) SearchOrgTypes(c *models.ReqContext) response.Response {
	query := c.Query("query")
	...
	cmd := &dtos.SearchOrgTypeMsg{
		Query:   query,
		Page:    int64(page),
		PerPage: int64(perPage),
	}
	url := fmt.Sprintf("%sapi/orgtypes?query=%s&page=%d&perPage=%d", hs.ResourceService.GetConfig().ResourceUrl, query, page, perPage)
	req := &resources.RestRequest{
		Url:        url,
		Request:    nil,
		HttpMethod: http.MethodGet,
