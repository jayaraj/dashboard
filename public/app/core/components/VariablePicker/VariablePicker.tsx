...
export const VariablePicker = forwardRef<HTMLDivElement, VariablePickerProps>(
  ({ value, onChange, placeholder = 'Search variables...', style, options = [], enableCreate = false }, ref) => {
    ...
    const handleInputKeyDown = (e: React.KeyboardEvent<Element>) => {
      if (e.key === 'Enter' && inputValue && !variableOptions.find((opt) => opt.value === inputValue)) {
        if (enableCreate) {
    ...
    return (
      <div ref={ref} style={{ width: '100%', ...style }}>
        ...
        {enableCreate && inputValue && !variableOptions.find((opt) => opt.value === inputValue) && (
          <Button variant="secondary" icon="plus" fullWidth onClick={handleAddOption} style={{ marginTop: 6 }}>
            Add
          </Button>
        )}
      </div>
