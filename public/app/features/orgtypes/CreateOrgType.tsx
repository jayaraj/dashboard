...
export class CreateOrgType extends PureComponent<Props, State> {
  state: State = {
    type: '',
    configuration: {
      elements: [],
  ...
  create = async () => {
    ...
    const result = await getBackendSrv().post('/api/orgtypes', {
      type,
      configuration,
    });
    if (result.id) {
      locationService.push(`/org/orgtypes/edit/${result.id}`);
    }
  };
  ...
  render() {
    ...
    return (
      <Page navModel={navModel}>
        <Page.Contents>
          <div>
            <h3 className="page-sub-heading">New OrgType</h3>

            <div className="gf-form-org">
              <div className="gf-form-org">
                <FormField
                  className="gf-form"
                  label="Type"
...
export default connect(mapStateToProps)(CreateOrgType);
