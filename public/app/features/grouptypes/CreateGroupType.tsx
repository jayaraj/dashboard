...
export class CreateGroupType extends PureComponent<Props, State> {
  state: State = {
    type: '',
    configuration: {
      elements: [],
  ...
  create = async () => {
    ...
    const result = await getBackendSrv().post('/api/grouptypes', {
      type,
      configuration,
    });
    if (result.id) {
      locationService.push(`/org/grouptypes/edit/${result.id}`);
    }
  };
  ...
  render() {
    ...
    return (
      <Page navModel={navModel}>
        <Page.Contents>
          <div>
            <h3 className="page-sub-heading">New GroupType</h3>
...
export default connect(mapStateToProps)(CreateGroupType);
