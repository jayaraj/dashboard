...
const alertsSlice = createSlice({
  ...
  reducers: {
    alertsByNameLoaded: (state, action: PayloadAction<{ name: string; alerts: Alert[] }>): AlertsState => {
      return {
        ...state,
        alertsByNameHasFetched: { ...state.alertsByNameHasFetched, [action.payload.name]: true },
        alertsByName: { ...state.alertsByName, [action.payload.name]: action.payload.alerts },
      };
    },
    alertsByStateLoaded: (
      state,
      action: PayloadAction<{ alertingState: AlertingState; alerts: Alert[] }>
    ): AlertsState => {
      return {
        ...state,
        alertsByStateHasFetched: { ...state.alertsByStateHasFetched, [action.payload.alertingState]: true },
        alertsByState: { ...state.alertsByState, [action.payload.alertingState]: action.payload.alerts },
      };
    },
    setAlertsByNameFetched: (state, action: PayloadAction<{ name: string; fetched: boolean }>): AlertsState => {
      return {
        ...state,
        alertsByNameHasFetched: { ...state.alertsByNameHasFetched, [action.payload.name]: action.payload.fetched },
      };
    },
    setAlertsByStateFetched: (
      state,
      action: PayloadAction<{ alertingState: AlertingState; fetched: boolean }>
    ): AlertsState => {
      return {
        ...state,
        alertsByStateHasFetched: {
          ...state.alertsByStateHasFetched,
          [action.payload.alertingState]: action.payload.fetched,
        },
      };
    },
    setAlertsByNameStats: (state, action: PayloadAction<{ name: string; stats: AlertStats }>): AlertsState => {
      return {
        ...state,
        alertsByNameStats: { ...state.alertsByNameStats, [action.payload.name]: action.payload.stats },
      };
    },
    setAlertsByStateStats: (
      state,
      action: PayloadAction<{ alertingState: AlertingState; stats: AlertStats }>
    ): AlertsState => {
      return {
        ...state,
        alertsByStateStats: { ...state.alertsByStateStats, [action.payload.alertingState]: action.payload.stats },
      };
    },
    ...
    setAlertsByStatePage: (
      state,
      action: PayloadAction<{ alertingState: AlertingState; page: number }>
    ): AlertsState => {
      return {
        ...state,
        alertsByStatePage: { ...state.alertsByStatePage, [action.payload.alertingState]: action.payload.page },
      };
    },
  },
});
...
export const {
  ...
  setAlertDefinitionsSearchQuery,
  setAlertDefinitionsFetched,
