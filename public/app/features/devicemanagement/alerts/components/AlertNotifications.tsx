...
import { AlertDefinition, UpdateNotificationDTO } from 'app/types/devicemanagement/alert';
...
export const AlertNotifications: FC<Props> = ({
  ...
  const canWrite = contextSrv.hasPermission('alerts:read');
  ...
  useEffect(() => {
    if (alertNotification) {
      ...
      if (alertNotification.configuration) {
        Object.keys(alertNotification.configuration).forEach(function (key) {
          config[key] = alertNotification.configuration[key];
        });
      }
    ...
    >
      {isLoading ? (
        ...
        <HorizontalGroup align="normal">
          <VerticalGroup>
            <InlineField className={styles.checkbox} label={'Email'} labelWidth={20}>
              <Switch
                value={updatedConfiguration['email']}
                onChange={() => {
                  onChange('email', !updatedConfiguration['email']);
                }}
              />
            </InlineField>
            <InlineField className={styles.checkbox} label={'Whatsapp'} labelWidth={20}>
              <Switch
                value={updatedConfiguration['whatsapp']}
                onChange={() => {
                  onChange('whatsapp', !updatedConfiguration['whatsapp']);
                }}
              />
            </InlineField>
          </VerticalGroup>
          ...
          <VerticalGroup>
            <InlineField className={styles.checkbox} label={'SMS'} labelWidth={20}>
              <Switch
                value={updatedConfiguration['sms']}
                onChange={() => {
                  onChange('sms', !updatedConfiguration['sms']);
                }}
              />
            </InlineField>
          </VerticalGroup>
