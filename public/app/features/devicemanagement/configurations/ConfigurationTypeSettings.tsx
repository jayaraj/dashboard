...
export const ConfigurationTypeSettings = ({ configurationType, updateConfigurationType }: Props) => {
  const canWrite = contextSrv.hasPermission('configurations:write');
  ...
  useEffect(() => {
    const elements = configurationType.configuration
      ...
      : [];
    const sections = configurationType.configuration
      ? JSON.parse(JSON.stringify(configurationType.configuration.sections))
      : [];
    setConfiguration({
      elements: [...elements],
      sections: [...sections],
