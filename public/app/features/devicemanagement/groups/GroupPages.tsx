...
export class GroupPages extends PureComponent<Props, State> {
  ...
  getCurrentPage() {
    const pages = ['groups', 'users', 'resources', 'alerts', 'settings'];
  ...
  renderPage(): React.ReactNode {
    ...
    switch (currentPage) {
      ...
      case PageTypes.Alerts:
        return <AlertList association={'group'} associationReference={group!.path} />;
  ...
  render() {
    ...
    const grpIds = group ? group.path.split(',') : [];
    const grpNames = group ? group.pathname!.split('/') : [];
          ...
          <span>
            ...
            {grpNames.map((name, i) => {
              return (
                <span key={i}>
                  /
                  <a href={`org/groups/edit/${grpIds[i + 1]}`} style={{ textDecoration: 'underline' }}>
                    {name}
                  </a>
                </span>
              );
