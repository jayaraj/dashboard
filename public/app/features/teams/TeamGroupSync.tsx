...
export const TeamSyncUpgradeContent = ({ action }: { action?: UpgradeContentProps['action'] }) => {
  ...
  return (
    <UpgradeContent
      action={action}
      listItems={[
        'Stop managing user access in two places - assign users to groups in SAML, LDAP or Oauth, and manage access at a Team level in Dashboard',
      ...
      description={
        'Team Sync makes it easier for you to manage users’ access in Dashboard, by immediately updating each user’s Dashboard teams and permissions based on their single sign-on group membership, instead of when users sign in.'
      }
    />
  );
};
