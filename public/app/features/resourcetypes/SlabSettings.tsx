...
export const SlabSettings: FC<Props> = ({ slab, deleteSlab, updateSlab }) => {
  ...
  const onSubmit = (data: Slab) => {
    ...
    if (id) {
      dispatch(updateSlab(id, Number(tax), updatedRates));
    } else {
      dispatch(createSlab(Number(tax), updatedRates));
    }
  };
    ...
    <FormProvider {...formSlab}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <FieldSet label={`${slab?.id === 0 ? 'Create slabs' : 'Update slabs'}`}>
          <Field
            label="Tax"
            description="tax applicable to these slabs values between (0 - 1)"
            disabled={!canWrite}
          >
