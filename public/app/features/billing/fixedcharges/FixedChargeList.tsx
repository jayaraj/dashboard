...
export class FixedChargeList extends PureComponent<Props> {
  ...
  renderFixedCharge(fixedCharge: FixedCharge) {
    ...
    const canRead = contextSrv.hasAccess(AccessControlAction.ActionFixedChargesRead, fallback);
  ...
  renderEmptyList() {
    const fallback = contextSrv.hasRole('ServerAdmin') || contextSrv.hasRole('Admin');
    const canCreate = contextSrv.hasAccess(AccessControlAction.ActionFixedChargesCreate, fallback)
    return <EmptyListCTA  title="No org-wide charges are available." buttonIcon="fixed-charge"  buttonDisabled={!canCreate} buttonLink="/org/fixedcharges/new" buttonTitle="Create org-wide charges" />;
  }

  renderFixedChargeList() {
    const { fixedCharges } = this.props;
    const fallback = contextSrv.hasRole('ServerAdmin') || contextSrv.hasRole('Admin');
    const canWrite = contextSrv.hasAccess(AccessControlAction.ActionFixedChargesCreate, fallback);
