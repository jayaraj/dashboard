...
import { AccessControlAction, TRANSACTION_TYPES, ContextDTO} from 'app/types';
...
export const CreateTransaction: FC<Props> = ({ 
  isOpen, 
  onCancel, 
  createTransaction }) => {
          ...
          <FieldSet>
            <div className={classNames([styles.flexRow, styles.alignBaseline])}>
              ...
              >
                <InputControl
                  name="type"
                  render={({field: {onChange, ...field}}) => <Select {...field} onChange={(value) => onChange(value.value)} options={TRANSACTION_TYPES} width={40}/>}
                  control={control}
                  rules={{ required: true }}
                />
              </Field>
