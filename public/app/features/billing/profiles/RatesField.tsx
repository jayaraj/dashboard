...
import { Slab } from 'app/types/billing/profile';

interface Props {
  className?: string;
}

const RatesField = ({ className }: Props) => {
            ...
            {rates.map((rate, index) => {
              return (
                <div className={styles.row} key={`${index}`} data-testid="rate">
                  <Field label="From" description="lower range" disabled={!canWrite}>
                    <Input
                  ...
                  {!rate.final && (
                    <Field label="To" description="higher range" disabled={!canWrite}>
                      <Input
                  ...
                  <Field label="Amount" description="Cost" disabled={!canWrite}>
                    <Input
                  ...
                  <Field label="Description" description="description" disabled={!canWrite}>
                    <Input
                  ...
                  <Field label="AboveOnly" description="is gt only" disabled={!canWrite}>
                    <Checkbox
                      ...
                      {...register(`rates.${index}.final`)}
                      defaultChecked={rate.final}
                      onChange={(e) => {
